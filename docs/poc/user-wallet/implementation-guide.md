...
> **Archive notice (2026-07-24):** This guide governed the now-paused
> user-wallet proof of concept. Its historical `IMPLEMENTATION-READY` status
> does not authorize current Wallet v3 work. See the [archive index](README.md)
...
- Build greenfield PoC code only under `contracts/poc/userWallet/` and `tests/poc/userWallet/`.
...
- Record material choices, failed assumptions, and scope deviations in `docs/poc/user-wallet/BUILDLOG.md`.

Allowed writes:

- `contracts/poc/userWallet/**`
- `tests/poc/userWallet/**`
- `docs/poc/user-wallet/BUILDLOG.md`
- `docs/poc/user-wallet/POC_RESULTS.md`
- `docs/poc/user-wallet/implementation-guide.md` and `docs/poc/user-wallet/user-wallet.md` only for an explicitly reviewed scope correction
...
`tests/poc/userWallet/conftest.py` may deploy small bounded inline EVM return-data shims for malformed and oversized low-level-call cases that Vyper's typed ABI cannot express. Document each shim next to its bytecode; it is test machinery, not a new wallet component.
...
```text
python -m pytest --fork=local --ignore=tests/poc/userWallet/gas tests/poc/userWallet/ -q
