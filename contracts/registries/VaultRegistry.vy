...
def setDefaultTargetVaultToken(_vaultAddr: address, _targetVaultToken: address):
    assert addys._isSwitchboardAddr(msg.sender) # dev: no perms
    assert registry._isValidAddr(_vaultAddr) # dev: invalid vault addr
    assert self.isApprovedVaultToken[_vaultAddr][_targetVaultToken] or _targetVaultToken == empty(address) # dev: vault token not approved
    config: VaultConfig = self.vaultConfigs[_vaultAddr]
...
def initializeVaultConfig(
    ...
    if _defaultTargetVaultToken != empty(address):
        ...
        for vaultToken: address in _approvedVaultTokens:
            if vaultToken == _defaultTargetVaultToken:
                isApproved = True
                break
        assert isApproved # dev: default target vault token not in approved list
    ...
    log ShouldAutoDepositSet(vaultAddr=_vaultAddr, shouldAutoDeposit=_shouldAutoDeposit)
    if _defaultTargetVaultToken != empty(address):
        log DefaultTargetVaultTokenSet(vaultAddr=_vaultAddr, targetVaultToken=_defaultTargetVaultToken)
...
def getDepositConfig(_vaultAddr: address) -> (bool, uint256, bool, address):
    """
    Returns all deposit-related config values in a single call for efficiency.
    Returns: (canDeposit, maxDepositAmount, shouldAutoDeposit, defaultTargetVaultToken)
    """
    config: VaultConfig = self.vaultConfigs[_vaultAddr]
...
def getVaultActionDataWithFrozenStatus(_legoId: uint256, _signer: address, _vaultAddr: address) -> (VaultActionData, bool):
    """
    Returns VaultActionData and isVaultOpsFrozen in a single call for efficiency.
    Returns: (VaultActionData, isVaultOpsFrozen)
    """
    ...
    ad: VaultActionData = VaultActionData(
        ledger = a.ledger,
    ...
    isVaultOpsFrozen: bool = self.vaultConfigs[_vaultAddr].isVaultOpsFrozen
    return ad, isVaultOpsFrozen
...
def getLegoAndSnapshotConfig(_vaultToken: address, _vaultAddr: address) -> (address, SnapShotPriceConfig):
    """
    Returns lego address and snapshot config in a single call for efficiency.
    Returns: (legoAddr, SnapShotPriceConfig)
    """
    legoAddr: address = self._getLegoDataFromVaultToken(_vaultToken)[1]
    config: SnapShotPriceConfig = self.vaultConfigs[_vaultAddr].snapShotPriceConfig
    return legoAddr, config
