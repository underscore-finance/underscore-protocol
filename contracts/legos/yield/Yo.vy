...
interface YoRegistry:
    def isYoVault(_vault: address) -> bool: view
...
YO_REGISTRY: public(immutable(address))
MAX_TOKEN_PATH: constant(uint256) = 5


@deploy
def __init__(_undyHq: address, _yoRegistry: address):
    addys.__init__(_undyHq)
    yld.__init__(False)

    assert _yoRegistry != empty(address) # dev: invalid addr
    YO_REGISTRY = _yoRegistry
...
def _getVaultTokenOnDeposit(_asset: address, _vaultAddr: address, _ledger: address, _legoBook: address) -> address:
    ...
    # not yet registered, call yo directly to get asset
    if asset == empty(address) and self._isValidYoVault(_vaultAddr):
...
def _getAssetOnWithdraw(_vaultToken: address, _ledger: address, _legoBook: address) -> address:
    ...
    # not yet registered, call yo directly to get asset
    if asset == empty(address) and self._isValidYoVault(_vaultToken):
...
@view
@internal
def _isValidYoVault(_vaultToken: address) -> bool:
    return staticcall YoRegistry(YO_REGISTRY).isYoVault(_vaultToken)
...
def _isValidAssetOpportunity(_asset: address, _vaultAddr: address) -> bool:
    return self._isValidYoVault(_vaultAddr) and staticcall IERC4626(_vaultAddr).asset() == _asset
