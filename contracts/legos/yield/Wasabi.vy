...
def _getVaultTokenOnDeposit(_asset: address, _vaultAddr: address, _ledger: address, _legoBook: address) -> address:
    ...
    if asset == empty(address):
        ...
        assert self._isValidWasabiVault(asset, _vaultAddr) # dev: invalid vault token
        isRegistered = False
...
def _getAssetOnWithdraw(_vaultToken: address, _ledger: address, _legoBook: address) -> address:
    ...
    if asset == empty(address):
        ...
        assert self._isValidWasabiVault(asset, _vaultToken) # dev: invalid vault token
        isRegistered = False
...
@view
@external
def isValidWasabiVault(_vaultToken: address) -> bool:
    asset: address = staticcall IERC4626(_vaultToken).asset()
    return self._isValidWasabiVault(asset, _vaultToken)


@view
@internal
def _isValidWasabiVault(_asset: address, _vaultToken: address) -> bool:
...
def _isValidAssetOpportunity(_asset: address, _vaultAddr: address) -> bool:
    return self._isValidWasabiVault(_asset, _vaultAddr)
