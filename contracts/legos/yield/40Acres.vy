...
def _getVaultTokenOnDeposit(_asset: address, _vaultAddr: address, _ledger: address, _legoBook: address) -> address:
    ...
    # not yet registered, call 40 acres directly to get asset
    if asset == empty(address) and self._isValid40AcresVault(_vaultAddr):
...
def _getAssetOnWithdraw(_vaultToken: address, _ledger: address, _legoBook: address) -> address:
    ...
    # not yet registered, call 40 acres directly to get asset
    if asset == empty(address) and self._isValid40AcresVault(_vaultToken):
...
def _isVaultToken(_vaultToken: address) -> bool:
    if yld.vaultToAsset[_vaultToken] != empty(address):
        return True
    return self._isValid40AcresVault(_vaultToken)


@view
@external
def isValid40AcresVault(_vaultToken: address) -> bool:
    return self._isValid40AcresVault(_vaultToken)


@view
@internal
def _isValid40AcresVault(_vaultToken: address) -> bool:
    return _vaultToken == FORTY_ACRES_USDC_VAULT
...
def _isValidAssetOpportunity(_asset: address, _vaultAddr: address) -> bool:
    return self._isValid40AcresVault(_vaultAddr) and staticcall IERC4626(_vaultAddr).asset() == _asset
