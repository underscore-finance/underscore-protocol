...
interface TokemakRegistry:
    def isVault(_vault: address) -> bool: view

event TokemakDeposit:
...
TOKEMAK_REGISTRY: public(immutable(address))
MAX_TOKEN_PATH: constant(uint256) = 5


@deploy
def __init__(_undyHq: address, _tokemakRegistry: address):
    addys.__init__(_undyHq)
    yld.__init__(False)

    assert _tokemakRegistry != empty(address) # dev: invalid addr
    TOKEMAK_REGISTRY = _tokemakRegistry
...
def _getVaultTokenOnDeposit(_asset: address, _vaultAddr: address, _ledger: address, _legoBook: address) -> address:
    ...
    # not yet registered, call tokemak directly to get asset
    if asset == empty(address) and self._isValidTokemakVault(_vaultAddr):
...
def _getAssetOnWithdraw(_vaultToken: address, _ledger: address, _legoBook: address) -> address:
    ...
    # not yet registered, call tokemak directly to get asset
    if asset == empty(address) and self._isValidTokemakVault(_vaultToken):
...
def _isVaultToken(_vaultToken: address) -> bool:
    if yld.vaultToAsset[_vaultToken] != empty(address):
        return True
    return self._isValidTokemakVault(_vaultToken)


@view
@external
def isValidTokemakVault(_vaultToken: address) -> bool:
    return self._isValidTokemakVault(_vaultToken)


@view
@internal
def _isValidTokemakVault(_vaultToken: address) -> bool:
    return staticcall TokemakRegistry(TOKEMAK_REGISTRY).isVault(_vaultToken)
...
def _isValidAssetOpportunity(_asset: address, _vaultAddr: address) -> bool:
    return self._isValidTokemakVault(_vaultAddr) and staticcall IERC4626(_vaultAddr).asset() == _asset
