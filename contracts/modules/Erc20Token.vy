...
def __init__(
    ...
    if _initialGov != empty(address):
        assert _undyHq == empty(address) # dev: cannot set initial gov and undy hq
        self.tempGov = _initialGov

    # set undy hq
    if _undyHq != empty(address):
        assert self._isValidNewUndyHq(_undyHq, empty(address)) # dev: invalid undy hq
        assert _initialGov == empty(address) # dev: cannot set initial gov and undy hq
        self.undyHq = _undyHq
...
def _isValidNewUndyHq(_newHq: address, _prevHq: address) -> bool:
    ...
    # tokens must be set
    if staticcall UndyHq(_newHq).undyToken() == empty(address):
        return False
...
def finishTokenSetup(_newHq: address, _timeLock: uint256 = 0) -> bool:
    ...
    # set hq
    assert self._isValidNewUndyHq(_newHq, prevHq) # dev: invalid undy hq
    self.undyHq = _newHq
    ...
    log InitialUndyHqSet(hq=_newHq, timeLock=timeLock)
    return True
