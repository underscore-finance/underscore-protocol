...
def test_avantis_deposit_max(
    ...
    testLegoDeposit(lego_avantis, asset, vault_token)


@pytest.mark.parametrize("token_str", TEST_ASSETS)
@pytest.always
def test_avantis_deposit_partial(
    token_str,
    testLegoDeposit,
    getVaultToken,
    bob_user_wallet,
    ...
    testLegoDeposit(lego_avantis, asset, vault_token, amount // 2)


@pytest.mark.parametrize("token_str", TEST_ASSETS)
@pytest.always
def test_avantis_withdraw_max(
    token_str,
    setupWithdrawal,
    lego_avantis,
    getVaultToken,
    ...
    lego_id = lego_book.getRegId(lego_avantis)
    vault_token = getVaultToken(token_str)
...
@pytest.mark.parametrize("token_str", TEST_ASSETS)
@pytest.always
def test_avantis_withdraw_partial(
    token_str,
    setupWithdrawal,
    lego_avantis,
    getVaultToken,
...
@pytest.mark.parametrize("token_str", TEST_ASSETS)
@pytest.always
def test_avantis_view_functions(
    token_str,
    getVaultToken,
    lego_avantis,
    ...
    vault_token = getVaultToken(token_str)
    testLegoViewFunctions(lego_avantis, vault_token, token_str)
