...
def getVaultToken(fork, alpha_token_vault):
    def getVaultToken(_token_str):
        if fork == "local":
            if _token_str == "ALPHA":
                return alpha_token_vault
            else:
...
def test_fluid_deposit_max(
    ...
    testLegoDeposit(lego_book.getRegId(lego_fluid), asset, vault_token)
...
def test_fluid_deposit_partial(
    ...
    testLegoDeposit(lego_book.getRegId(lego_fluid), asset, vault_token, amount // 2)
