...
@pytest.fixture(scope="session")
def lego_aave_v3(fork, lego_book, undy_hq_deploy, governance, mock_aave_v3_pool, mock_ripe):
    ...
    addr = boa.load("contracts/legos/yield/AaveV3.vy", undy_hq_deploy,
                    AAVE_V3_POOL, AAVE_V3_ADDRESS_PROVIDER, RIPE_HQ, name="lego_aave_v3")
...
@pytest.fixture(scope="session")
def lego_moonwell(mock_lego_registry, fork, lego_book, undy_hq_deploy, weth, governance, mock_ripe):
    ...
    addr = boa.load("contracts/legos/yield/Moonwell.vy", undy_hq_deploy,
                    MOONWELL_COMPTROLLER, weth, RIPE_HQ, name="lego_moonwell")
...
@pytest.fixture(scope="session")
def lego_compound_v3(mock_lego_registry, fork, lego_book, undy_hq_deploy, governance, mock_ripe):
    ...
    addr = boa.load("contracts/legos/yield/CompoundV3.vy", undy_hq_deploy,
                    COMPOUND_V3_CONFIGURATOR, COMPOUND_V3_REWARDS, RIPE_HQ, name="lego_compound_v3")
...
@pytest.fixture(scope="session")
def lego_morpho(fork, lego_book, undy_hq_deploy, governance, mock_lego_registry, mock_ripe):
    ...
    addr = boa.load("contracts/legos/yield/Morpho.vy", undy_hq_deploy, MORPHO_FACTORY,
                    MORPHO_FACTORY_LEGACY, MORPHO_REWARDS, RIPE_HQ, name="lego_morpho")
...
@pytest.fixture(scope="session")
def lego_euler(fork, lego_book, undy_hq_deploy, governance, mock_lego_registry, mock_ripe):
    ...
    addr = boa.load("contracts/legos/yield/Euler.vy", undy_hq_deploy, EULER_EVAULT_FACTORY,
                    EULER_EARN_FACTORY, EULER_REWARDS, RIPE_HQ, name="lego_euler")
...
@pytest.fixture(scope="session")
def lego_sky_psm(fork, lego_book, undy_hq_deploy, governance, mock_lego_registry, mock_ripe):
    SKY_PSM = mock_lego_registry if fork == "local" else INTEGRATION_ADDYS[fork]["SKY_PSM"]
    RIPE_HQ = mock_ripe if fork == "local" else INTEGRATION_ADDYS[fork]["RIPE_HQ_V1"]
    addr = boa.load("contracts/legos/yield/SkyPsm.vy", undy_hq_deploy, SKY_PSM, RIPE_HQ, name="lego_sky_psm")
...
@pytest.fixture(scope="session")
def lego_underscore(fork, lego_book, undy_hq_deploy, governance, alpha_token, mock_ripe):
    RIPE_TOKEN = alpha_token if fork == "local" else TOKENS[fork]["RIPE"]
    RIPE_HQ = mock_ripe if fork == "local" else INTEGRATION_ADDYS[fork]["RIPE_HQ_V1"]
    addr = boa.load("contracts/legos/UnderscoreLego.vy", undy_hq_deploy, RIPE_TOKEN, RIPE_HQ, name="lego_underscore")
...
@pytest.fixture(scope="session")
def lego_wasabi(fork, lego_book, undy_hq_deploy, governance, mock_lego_registry, mock_ripe):
    ...
    addr = boa.load("contracts/legos/yield/Wasabi.vy", undy_hq_deploy,
                    WASABI_LONG_POOL, WASABI_SHORT_POOL, RIPE_HQ, name="lego_wasabi")
...
@pytest.fixture(scope="session")
def lego_avantis(fork, lego_book, undy_hq_deploy, governance, mock_lego_registry, mock_ripe):
    AVANTIS_USDC = mock_lego_registry if fork == "local" else TOKENS[fork]["AVANTIS_USDC"]
    RIPE_HQ = mock_ripe if fork == "local" else INTEGRATION_ADDYS[fork]["RIPE_HQ_V1"]
    addr = boa.load("contracts/legos/yield/Avantis.vy", undy_hq_deploy, AVANTIS_USDC, RIPE_HQ, name="lego_avantis")
