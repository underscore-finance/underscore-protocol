...
def test_payee_example_test(createGlobalPayeeSettings, charlie, alpha_token, bravo_token, bob, createPayeeLimits, createPayeeSettings, sentinel, user_wallet, user_wallet_config, alice, paymaster):
    ...
    # not payee
    assert not sentinel.isValidPayee(user_wallet, charlie, alpha_token, 1, 1)

    # payee -- not primary asset
    assert not sentinel.isValidPayee(user_wallet, alice, bravo_token, 9, 1)

    # payee -- primary asset
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 9, 1)

    # payee -- over limit
    assert not sentinel.isValidPayee(user_wallet, alice, alpha_token, 11, 1)
...
def test_whitelisted_recipient_always_valid(alpha_token, bravo_token, delta_token, sentinel, user_wallet, user_wallet_config, alice, paymaster):
    ...
    assert sentinel.isValidPayee(user_wallet, alice, bravo_token, 999_999_999, 999_999_999)
    assert sentinel.isValidPayee(user_wallet, alice, delta_token, 1, 1)
    assert sentinel.isValidPayee(user_wallet, alice, ZERO_ADDRESS, 0, 0)  # even with zero address


def test_non_whitelisted_non_payee_invalid(alpha_token, sentinel, user_wallet, sally):
    # sally is neither whitelisted nor a payee
    assert not sentinel.isValidPayee(user_wallet, sally, alpha_token, 1, 1)
...
def test_payee_not_yet_active(createPayeeSettings, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
...
def test_payee_active(createPayeeSettings, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
...
def test_payee_expired(createPayeeSettings, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
...
def test_primary_asset_only_restriction(createPayeeSettings, alpha_token, bravo_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert not sentinel.isValidPayee(user_wallet, alice, bravo_token, 1, 1)


def test_no_primary_asset_restriction(createPayeeSettings, alpha_token, bravo_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 1, 1)
    assert sentinel.isValidPayee(user_wallet, alice, bravo_token, 1, 1)


# transaction limits


def test_max_txs_per_period_limit(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_usd_per_tx_cap(createPayeeSettings, createPayeeLimits, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 1, 99)
    
    # should not be valid over limit
    assert not sentinel.isValidPayee(user_wallet, alice, alpha_token, 1, 101)


def test_usd_per_period_cap(createGlobalPayeeSettings, createPayeeSettings, createPayeeLimits, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_usd_lifetime_cap(createGlobalPayeeSettings, createPayeeSettings, createPayeeLimits, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_unit_per_tx_cap(createPayeeSettings, createPayeeLimits, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    # should be valid under limit
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 999, 1)
    
    # should not be valid over limit
    assert not sentinel.isValidPayee(user_wallet, alice, alpha_token, 1001, 1)


def test_unit_limits_only_apply_to_primary_asset(createPayeeSettings, createPayeeLimits, alpha_token, bravo_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert not sentinel.isValidPayee(user_wallet, alice, alpha_token, 101, 1)
    
    # unit limit should NOT apply to other assets
    assert sentinel.isValidPayee(user_wallet, alice, bravo_token, 1000, 1)


def test_unit_per_period_cap(createGlobalPayeeSettings, createPayeeSettings, createPayeeLimits, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_unit_lifetime_cap(createGlobalPayeeSettings, createPayeeSettings, createPayeeLimits, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_period_reset_after_period_ends(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_first_transaction_initializes_period(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
    # add payee
...
def test_fail_on_zero_price_payee_setting(createPayeeSettings, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert not sentinel.isValidPayee(user_wallet, alice, alpha_token, 100, 0)
    
    # should succeed with non-zero price
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 100, 1)


def test_fail_on_zero_price_global_setting(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_payee_data_updates_correctly(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
    # add payee
    new_payee_settings = createPayeeSettings(_primaryAsset=alpha_token)
    user_wallet_config.addPayee(alice, new_payee_settings, sender=paymaster.address)
...
def test_non_primary_asset_does_not_update_units(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, bravo_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_multiple_limits_most_restrictive_applies(createGlobalPayeeSettings, createPayeeSettings, createPayeeLimits, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert not sentinel.isValidPayee(user_wallet, alice, alpha_token, 50, 600)
    
    # should fail on unit limit
    assert not sentinel.isValidPayee(user_wallet, alice, alpha_token, 101, 100)
    
    # should pass when both limits satisfied
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 50, 100)


def test_global_tx_limits_apply_to_payees(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_zero_limit_means_unlimited(createPayeeSettings, createPayeeLimits, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 999_999_999, 999_999_999)


def test_empty_primary_asset_with_only_primary_false(createPayeeSettings, alpha_token, bravo_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 100, 100)
    assert sentinel.isValidPayee(user_wallet, alice, bravo_token, 100, 100)


# additional scenarios


def test_exact_limit_boundaries(createPayeeSettings, createPayeeLimits, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
...
def test_transaction_at_exact_period_end(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_zero_amount_transaction(createPayeeSettings, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
    ...
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 0, 0)
    
    # zero amount with non-zero USD value should work
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 0, 100)


def test_cooldown_equals_period_length(createGlobalPayeeSettings, user_wallet, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_multiple_period_resets(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_payee_expires_at_current_block(createPayeeSettings, alpha_token, alice, sentinel, user_wallet, user_wallet_config, paymaster):
...
def test_global_and_payee_cooldowns_both_apply(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_very_long_inactive_period(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, alpha_token, alice, sentinel, user_wallet_config, paymaster):
...
def test_all_limits_zero_means_unlimited(createGlobalPayeeSettings, user_wallet, createPayeeSettings, createPayeeLimits, alpha_token, alice, sentinel, user_wallet_config, paymaster):
    ...
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 10**18, 10**18)
    assert sentinel.isValidPayee(user_wallet, alice, alpha_token, 10**24, 10**24)


def test_lifetime_limits_persist_across_period_resets(createGlobalPayeeSettings, createPayeeSettings, createPayeeData, createPayeeLimits, alpha_token, alice, sentinel, user_wallet_config, paymaster):
