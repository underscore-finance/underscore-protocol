...
def test_owner_can_manage_all_whitelist_actions(paymaster, user_wallet, bob):
    """Test that the owner has permission for all whitelist actions"""
    # Owner should have permission for all actions
    assert paymaster.canManageWhitelist(user_wallet, bob, WHITELIST_ACTION.ADD_PENDING)
    assert paymaster.canManageWhitelist(user_wallet, bob, WHITELIST_ACTION.CONFIRM_WHITELIST)
    assert paymaster.canManageWhitelist(user_wallet, bob, WHITELIST_ACTION.CANCEL_WHITELIST)
    assert paymaster.canManageWhitelist(user_wallet, bob, WHITELIST_ACTION.REMOVE_WHITELIST)


def test_manager_whitelist_permissions_individual(createGlobalManagerSettings, createWhitelistPerms, sally, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, high_command):
    ...
    assert paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.ADD_PENDING)
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.CONFIRM_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.CANCEL_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.REMOVE_WHITELIST)
    ...
    assert not paymaster.canManageWhitelist(user_wallet, sally, WHITELIST_ACTION.ADD_PENDING)
    assert paymaster.canManageWhitelist(user_wallet, sally, WHITELIST_ACTION.CONFIRM_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, sally, WHITELIST_ACTION.CANCEL_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, sally, WHITELIST_ACTION.REMOVE_WHITELIST)


def test_manager_whitelist_permissions_multiple(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, high_command):
    ...
    assert paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.ADD_PENDING)
    assert paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.CONFIRM_WHITELIST)
    assert paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.CANCEL_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.REMOVE_WHITELIST)


def test_global_whitelist_permissions_override(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, high_command):
...
def test_global_permissions_restrict_manager(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, high_command):
...
def test_non_manager_non_owner_denied(paymaster, user_wallet, charlie):
    """Test that non-manager/non-owner users are denied all whitelist actions"""
    assert not paymaster.canManageWhitelist(user_wallet, charlie, WHITELIST_ACTION.ADD_PENDING)
    assert not paymaster.canManageWhitelist(user_wallet, charlie, WHITELIST_ACTION.CONFIRM_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, charlie, WHITELIST_ACTION.CANCEL_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, charlie, WHITELIST_ACTION.REMOVE_WHITELIST)


def test_manager_with_all_permissions_false(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, high_command):
    ...
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.ADD_PENDING)
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.CONFIRM_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.CANCEL_WHITELIST)
    assert not paymaster.canManageWhitelist(user_wallet, alice, WHITELIST_ACTION.REMOVE_WHITELIST)


def test_whitelist_management_example(createGlobalManagerSettings, createWhitelistPerms, sally, charlie, bob, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, high_command):
...
def test_add_pending_whitelist_success(paymaster, user_wallet, user_wallet_config, bob, charlie):
    """Test successful pending whitelist creation"""
    # Add pending whitelist as owner
    paymaster.addPendingWhitelistAddr(user_wallet, charlie, sender=bob)
    ...
    log = filter_logs(paymaster, "WhitelistAddrPending")[0]
...
def test_add_pending_whitelist_with_timelock(paymaster, user_wallet, user_wallet_config, bob, alice):
    ...
    paymaster.addPendingWhitelistAddr(user_wallet, alice, sender=bob)
...
def test_add_pending_whitelist_invalid_addresses(paymaster, user_wallet, user_wallet_config, bob):
    """Test invalid address validation"""
    # Cannot add zero address
    with boa.reverts("invalid addr"):
        paymaster.addPendingWhitelistAddr(user_wallet, ZERO_ADDRESS, sender=bob)
    
    # Cannot add the wallet itself
    with boa.reverts("invalid addr"):
        paymaster.addPendingWhitelistAddr(user_wallet, user_wallet, sender=bob)
    
    # Cannot add the owner
    with boa.reverts("invalid addr"):
        paymaster.addPendingWhitelistAddr(user_wallet, bob, sender=bob)
    
    # Cannot add the wallet config
    with boa.reverts("invalid addr"):
        paymaster.addPendingWhitelistAddr(user_wallet, user_wallet_config.address, sender=bob)


def test_add_pending_whitelist_already_pending(paymaster, user_wallet, bob, charlie):
...
def test_add_pending_whitelist_already_whitelisted(paymaster, user_wallet, user_wallet_config, bob, alice):
    ...
    paymaster.confirmWhitelistAddr(user_wallet, alice, sender=bob)
...
def test_add_pending_whitelist_invalid_user_wallet(paymaster, bob, alice):
    """Test validation of user wallet"""
    # Try with random address that's not a user wallet
    with boa.reverts("invalid user wallet"):
        paymaster.addPendingWhitelistAddr(alice, bob, sender=bob)


def test_add_pending_whitelist_by_manager(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, charlie, high_command):
    ...
    paymaster.addPendingWhitelistAddr(user_wallet, charlie, sender=alice)
...
def test_confirm_whitelist_success(paymaster, user_wallet, user_wallet_config, bob, charlie):
    ...
    # Confirm the whitelist
    paymaster.confirmWhitelistAddr(user_wallet, charlie, sender=bob)
    ...
    # Verify event emission
    log = filter_logs(paymaster, "WhitelistAddrConfirmed")[0]
    assert log.user == user_wallet.address
    assert log.addr == charlie
...
def test_confirm_whitelist_timelock_not_reached(paymaster, user_wallet, user_wallet_config, bob, alice):
...
def test_confirm_whitelist_exactly_at_timelock(paymaster, user_wallet, user_wallet_config, bob, alice):
...
def test_confirm_whitelist_no_pending(paymaster, user_wallet, bob, charlie):
...
def test_confirm_whitelist_owner_changed(paymaster, user_wallet, user_wallet_config, bob, alice, charlie):
    ...
    with boa.reverts("owner must match"):
        paymaster.confirmWhitelistAddr(user_wallet, charlie, sender=alice)


def test_confirm_whitelist_invalid_user_wallet(paymaster, bob, alice):
    """Test validation of user wallet"""
    # Try with random address that's not a user wallet
    with boa.reverts("invalid user wallet"):
        paymaster.confirmWhitelistAddr(alice, bob, sender=bob)


def test_confirm_whitelist_by_manager(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, charlie, bob, high_command):
...
def test_confirm_whitelist_already_confirmed(paymaster, user_wallet, user_wallet_config, bob, alice):
...
def test_cancel_pending_whitelist_success(paymaster, user_wallet, user_wallet_config, bob, charlie):
    ...
    paymaster.cancelPendingWhitelistAddr(user_wallet, charlie, sender=bob)
    ...
    # Verify event emission
    log = filter_logs(paymaster, "WhitelistAddrCancelled")[0]
    assert log.user == user_wallet.address
    assert log.addr == charlie
    assert log.initiatedBlock == initiated_block
    assert log.confirmBlock == confirm_block
...
def test_cancel_pending_whitelist_no_pending(paymaster, user_wallet, bob, charlie):
...
def test_cancel_pending_whitelist_after_timelock(paymaster, user_wallet, user_wallet_config, bob, alice):
    ...
    paymaster.cancelPendingWhitelistAddr(user_wallet, alice, sender=bob)
...
def test_cancel_pending_whitelist_invalid_user_wallet(paymaster, bob, alice):
    """Test validation of user wallet"""
    # Try with random address that's not a user wallet
    with boa.reverts("invalid user wallet"):
        paymaster.cancelPendingWhitelistAddr(alice, bob, sender=bob)


def test_cancel_pending_whitelist_by_manager(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, charlie, bob, high_command):
    ...
    paymaster.cancelPendingWhitelistAddr(user_wallet, charlie, sender=alice)
...
def test_cancel_pending_whitelist_by_security_action(paymaster, user_wallet, user_wallet_config, bob, charlie, alice, mission_control, switchboard_alpha):
...
def test_cancel_pending_whitelist_no_permission(paymaster, user_wallet, bob, charlie, alice):
...
def test_cancel_then_add_again(paymaster, user_wallet, user_wallet_config, bob, charlie):
...
def test_cancel_pending_whitelist_already_confirmed(paymaster, user_wallet, user_wallet_config, bob, alice):
...
def test_remove_whitelist_success(paymaster, user_wallet, user_wallet_config, bob, charlie):
    ...
    paymaster.removeWhitelistAddr(user_wallet, charlie, sender=bob)
    
    # Verify no longer whitelisted
    assert user_wallet_config.indexOfWhitelist(charlie) == 0
    
    # Verify event emission
    log = filter_logs(paymaster, "WhitelistAddrRemoved")[0]
    assert log.user == user_wallet.address
    assert log.addr == charlie
    assert log.removedBy == bob


def test_remove_whitelist_not_whitelisted(paymaster, user_wallet, bob, charlie):
...
def test_remove_whitelist_after_removal(paymaster, user_wallet, user_wallet_config, bob, alice):
    """Test cannot remove already removed address"""
    # Add, confirm, and remove
    paymaster.addPendingWhitelistAddr(user_wallet, alice, sender=bob)
    boa.env.time_travel(blocks=user_wallet_config.timeLock())
    paymaster.confirmWhitelistAddr(user_wallet, alice, sender=bob)
    paymaster.removeWhitelistAddr(user_wallet, alice, sender=bob)
...
def test_remove_whitelist_invalid_user_wallet(paymaster, bob, alice):
    """Test validation of user wallet"""
    # Try with random address that's not a user wallet
    with boa.reverts("invalid user wallet"):
        paymaster.removeWhitelistAddr(alice, bob, sender=bob)


def test_remove_whitelist_by_manager(createGlobalManagerSettings, createWhitelistPerms, createManagerSettings, paymaster, user_wallet, user_wallet_config, alice, charlie, bob, high_command):
    ...
    paymaster.removeWhitelistAddr(user_wallet, charlie, sender=alice)
...
def test_remove_whitelist_by_security_action(paymaster, user_wallet, user_wallet_config, bob, charlie, alice, mission_control, switchboard_alpha):
...
def test_remove_whitelist_no_permission(paymaster, user_wallet, user_wallet_config, bob, charlie, alice):
...
def test_remove_then_add_again(paymaster, user_wallet, user_wallet_config, bob, charlie):
...
def test_remove_whitelist_pending_exists(paymaster, user_wallet, user_wallet_config, bob, alice):
