...
def test_agent_rebalance_yield_position_basic(
    setupAgentTestAsset,
    starter_agent,
    user_wallet,
    charlie,
    ...
    """Test AgentWrapper rebalanceYieldPosition function"""
    ...
    # Deposit through agent wrapper to create initial position
    _, _, vault_tokens, _ = starter_agent.depositForYield(
        user_wallet.address,
        1,  # from_lego_id
        alpha_token.address,
    ...
    # Now rebalance half to different vault
    rebalance_amount = vault_tokens // 2
    underlying_amount, to_vault_token, to_vault_tokens_received, max_usd_value = starter_agent.rebalanceYieldPosition(
        user_wallet.address,
        1,  # from_lego_id
        alpha_token_vault.address,
        1,  # to_lego_id (same lego, different vault)
    ...
    assert to_vault_tokens_received > 0
    assert max_usd_value > 0
    
    # Verify balances
    assert alpha_token_vault.balanceOf(user_wallet) == vault_tokens - rebalance_amount
